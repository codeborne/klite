...
class JdbcExtensionsTest {
  ...
  @Test fun whereExpr() {
    val where = whereConvert(values) + sql("exists (subselect)") + or("a" to "b", "array" any 123, "something" like "x%", "num" gte 1)
    ...
    expect(whereValues(where.values).toList()).toContainExactly("world", 1, 2, 3, "{}", "b", 123, "x%", 1)
  }
}
